...
bindable resources
*/
struct FVkShaderParamBufferInfo
{
	//dset layout info  
...
struct FVkShaderParamTextureInfo
{
	VkShaderStageFlags   stageFlags;
	uint32_t             binding;
	uint32_t  	         set;
...
struct FVkShaderParamPushConstantInfo
{
	VkShaderStageFlags   stageFlags;
	uint32_t             size;
};
...
{
public:
	using UniformBufferMap = std::unordered_map<std::string, FVkShaderParamBufferInfo>;
	using StorageBufferMap = std::unordered_map<std::string, FVkShaderParamBufferInfo>;
	using SamplerMap = std::unordered_map<std::string, FVkShaderParamTextureInfo>;
	using PushConstantMap = std::unordered_map<std::string, FVkShaderParamPushConstantInfo>;

	// API for the reflection process , use const char* to be compatible with SPIR-V reflection
public:
	void AddSampler(const char* name, const FVkShaderParamTextureInfo& info)
	{
		samplerMap.emplace(std::string(name), info);
	}

	void AddUniformBuffer(const char* name, const FVkShaderParamBufferInfo& info)
	{
		uniformBufferMap.emplace(std::string(name), info);
	}

	void AddStorageBuffer(const char* name, const FVkShaderParamBufferInfo& info)
	{
		storageBufferMap.emplace(std::string(name), info);
	}

	void AddPushConstant(const char* name, const FVkShaderParamPushConstantInfo& info)
	{
...
public:
	...
	VkDescriptorSetLayout descriptorSetLayout = VK_NULL_HANDLE;  //per-pipeline
