...
void FVkTexture::CreateTexture(ImageMetaInfo& imageInfo, void* data) {
	...
	//hardcode for now
	//both src and dst if we generate mipmaps
	imageConfig = {
	...
	//if(generateMipmaps)//always true for now;
	...
	VkDeviceSize imageSize = imageConfig.extent.width * imageConfig.extent.height * 4; //hardcode 4 channels
	...
	heapManager->UpdateImageMemory(imageConfig.memoryProperty, image, imageMemory, imageConfig.extent, imageSize, data);
...
void LoadImage(const char* path, ImageMetaInfo& imageInfo, void*& data) {
	...
	imageInfo = { (uint32_t)texWidth, (uint32_t)texHeight, 4 };
	data = pixels;

	std::cout << "load image size: " << texWidth << " " << texHeight << std::endl;
