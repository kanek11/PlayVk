...
public:
	FVkStaticMeshResource(SharedPtr<FVkDevice> device,
		StaticMeshArraysRef& vertexData, std::vector<uint16_t>& indices)
		: deviceRef(device), vertexDataRef(vertexData), indicesRef(indices) {
		createVkResource();
	};
	...
	StaticMeshArraysRef& vertexDataRef;
	std::vector<uint16_t>& indicesRef;
...
public:
	...
	{
		StaticMeshArraysRef vertexData = { positions, UVs, normals, tangents };
